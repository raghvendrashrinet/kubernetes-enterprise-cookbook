...
graph TD
    ...
    classDef issueStyle fill:#ffcccc,stroke:#cc0000,stroke-width:2px,color:#000;
    classDef solutionStyle fill:#d4edda,stroke:#28a745,stroke-width:2px,color:#000;
    classDef normalStyle fill:#e2e3e5,stroke:#383d41,stroke-width:1px,color:#000;

    %% Main Architecture Flow
    subgraph Issue1 [Issue 1: The 'Lagging Indicator' Problem]
        A[Incoming Traffic Burst] -->|Floods Event Broker/Queue| B(Kafka Topic / Azure Service Bus)
        B -->|Message backlog grows to 10,000| C{Native HPA Engine}
        C -->|Reads ONLY CPU/Memory usage| D(Pod is currently idle at 5% CPU)
        D -->|Result| E[NO SCALE EVENT OCCURS]:::issueStyle
        E -->|Consequence| F(Massive Data Processing Delay & Outage)
    end

    subgraph KEDASolution1 [KEDA Solution 1: Direct Event Metrics Scaling]
        G(KEDA Custom Scaler) -->|Directly counts unread message lag| B
        G -->|Reports exactly 10,000 pending items| H{KEDA Controller}
        H -->|Instantly scales out replicas ahead of time| I[App scales BEFORE CPU runs hot]:::solutionStyle
    end

    subgraph Issue2 [Issue 2: The 'Always Paying' Infrastructure Tax]
        J[0 Traffic / Empty Queues at 3:00 AM] --> K{Native HPA Engine}
        K -->|Hard Floor Limit: MinReplicas = 1| L[Must keep at least 1 Pod alive]:::issueStyle
        L -->|Consequence| M(Wasting Cloud Budget on Idle Infrastructure)
    end

    subgraph KEDASolution2 [KEDA Solution 2: Scale-To-Zero Cost Optimization]
        N(KEDA Scaler) -->|Detects Queue is completely empty| O{KEDA Controller}
        O -->|Gracefully tears down final active pod| P[Scale Down to 0 Pods]:::solutionStyle
        P -->|Result| Q(Zero Waste Cloud Budget Savings)
        R[First New Message Arrives] -->|Wakes Up| O
    end

    %% Connectors
    C -.->|Replaced/Augmented By| G
    K -.->|Replaced/Augmented By| N
```
